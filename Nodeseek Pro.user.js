        ...
        const lotteryReminder = {
            ...
            init(ctx) {
                ...
                const DEFAULT_NOTIFY = {
                    ...
                    bark: { enabled: false, key: "", server: "https://api.day.app", sound: "", icon: "" },
                ...
                async function sendBark(config, subject, text, url) {
                    if (!config.enabled || !config.key) return;
                    const server = requireHttpsUrl(config.server || "https://api.day.app").replace(/\/$/, "");
                    const payload = { title: subject, body: text, device_key: config.key, group: "NodeSeek抽奖" };
                    if (url) payload.url = url;
                    if (config.sound) payload.sound = config.sound;
                    if (config.icon) payload.icon = config.icon;
                    return postJson(server + "/push", payload);
                }
                ...
                async function sendDiscord(config, subject, text, url) {
                    if (!config.enabled || !config.webhook) return;
                    const webhook = requireHttpsUrl(config.webhook, ["discord.com", "discordapp.com"]);
                    return postJson(webhook, {
                        username: config.username || "NodeSeek抽奖助手",
                        content: "**" + subject + "**\n" + text + (url ? "\n" + url : "")
                    });
                }

                async function sendAllNotifications(subject, text, url) {
                    GM_notification({
                        title: subject,
                        text,
                        timeout: 0,
                ...
                function extractLuckyUrl(html) {
                    const doc = new DOMParser().parseFromString(html, "text/html");
                    for (const link of doc.querySelectorAll('a[href*="/lucky?"]')) {
                        try {
                            const url = new URL(link.getAttribute("href"), location.origin);
                            if (/(^|\.)nodeseek\.com$/i.test(url.hostname)) return url.href;
                        } catch { }
                    }
                    const match = doc.body?.textContent?.match(/https?:\/\/(?:www\.)?nodeseek\.com\/lucky\?[^\s"'<>)]*/i);
                    return match?.[0] || null;
                }
                ...
                function titleFromRoot(root) {
                    ...
                    return title || document.title.replace(/\s*[-|]\s*NodeSeek.*$/i, "").trim() || "抽奖活动";
                }
                ...
                function detectLottery(root, explicitPostUrl) {
                    ...
                    const text = normalizeLotteryText(root.textContent);
                    const luckyUrl = extractLuckyUrl(root.outerHTML || "");
                    const detected = !!luckyUrl
                        || /(?:抽奖|开奖|有奖|giveaway|raffle)/i.test(title)
                        || /(?:评论|留言|回复)[^。；;！？!?\n]{0,36}(?:抽奖|开奖|中奖)/i.test(text)
                        || /(?:抽取|送出)[^。；;！？!?\n]{0,24}(?:名|位|个)[^。；;！？!?\n]{0,18}(?:奖|鸡腿|礼物)/i.test(text);
                ...
                function hasMatchingUserComment(details) {
                    ...
                    return [...document.querySelectorAll(".comments .content-item,.comments .comment-item")].some(comment => {
                ...
                function parseLotteryResult(html, myUsername) {
                    const doc = new DOMParser().parseFromString(html, "text/html");
                    const scopedRoots = [...doc.querySelectorAll('[class*="winner"],[class*="result"],[class*="lucky"]')];
                    const roots = scopedRoots.length ? scopedRoots : [doc.querySelector("main") || doc.body];
                    const names = [];
                    const seen = new Set();
                    ...
                    if (!names.length) {
                        const matches = doc.body?.textContent?.match(/@([a-zA-Z0-9_\-\u4e00-\u9fa5]{2,20})/g) || [];
                ...
                async function processCurrentLuckyPage() {
                    ...
                    const reminder = reminders.find(value => {
                        if (!value.luckyUrl || value.resultNotified) return false;
                        try {
                ...
                function saveLotteryDetails(details) {
                    const reminders = getReminders();
                    const index = reminders.findIndex(value => postIdFromUrl(value.postUrl) === details.postId);
                ...
                function detailsForCurrentPost() {
                    const root = document.querySelector(".nsk-post") || document.querySelector("article.post-content") || document.body;
                    const details = detectLottery(root, canonicalPostUrl(location.href));
                    if (details) return details;
                    const postUrl = canonicalPostUrl(location.href);
                    return {
                        postId: postIdFromUrl(postUrl),
                        postUrl,
                        title: titleFromRoot(root),
                ...
                function openNotifyConfig() {
                    ...
                    const sections = [
                        ...
                        },
                        {
                            title: "Bark",
                            fields: [
                                { path: "bark.enabled", label: "启用", type: "checkbox" },
                                { path: "bark.key", label: "Device Key", type: "password" },
                                { path: "bark.server", label: "服务器" },
                                { path: "bark.sound", label: "提示音" },
                                { path: "bark.icon", label: "图标 URL" }
                            ]
                        },
                        {
                            title: "钉钉",
                        ...
                        },
                        {
                            title: "Discord",
                            fields: [
                                { path: "discord.enabled", label: "启用", type: "checkbox" },
                                { path: "discord.webhook", label: "Webhook", type: "password" },
                                { path: "discord.username", label: "机器人名称" }
                            ]
                        }
                    ];
                ...
                function createPanel() {
                    ...
                    panel.innerHTML =
                        ...
                        '<div class="nsx-lottery-toolbar">' +
                            '<button type="button" data-action="add">添加当前帖子</button>' +
                            '<button type="button" data-action="notify">通知</button>' +
                    ...
                    panel.querySelector('[data-action="add"]').addEventListener("click", addCurrentLottery);
                    panel.querySelector('[data-action="notify"]').addEventListener("click", openNotifyConfig);
                    panel.querySelector('[data-action="refresh"]').addEventListener("click", () => {
                        renderList();
                        checkReminders();
                    });
                    const addButton = panel.querySelector('[data-action="add"]');
                    addButton.disabled = !ctx.isPost;
                    addButton.title = ctx.isPost ? "保存当前帖子中的抽奖" : "请先打开帖子";
                    renderList();
                    return panel;
                }
